...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, codelen, hidden_size), requires_grad=False)

        self.blocks = nn.ModuleList([
            SiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_size, codesize)
    ...
    def initialize_weights(self):
        ...
        # TODO: Use 1D sin-cos positional embedding
...
def get_1d_sincos_pos_embed(embed_dim, codelen, cls_token=False, extra_tokens=0):
    """
    codelen: int of the sequence length
    return:
    pos_embed: [codelen, embed_dim] or [1+codelen, embed_dim] (w/ or w/o cls_token)
    """
    pos = np.arange(codelen, dtype=np.float32)
    pos_embed = get_1d_sincos_pos_embed_from_grid(embed_dim, pos)
    if cls_token and extra_tokens > 0:
        pos_embed = np.concatenate([np.zeros([extra_tokens, embed_dim]), pos_embed], axis=0)
    return pos_embed # [codelen, D]
